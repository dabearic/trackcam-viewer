...
test('parse full image', async ()=>{
    ...
    expect(value.prediction.score).approximately(0.8883,1e-5, "Score value")
    expect(value.prediction.category).toEqual(Category.HUMAN)
    expect(value.prediction.top5.size).toEqual(5)
    expect(value.prediction.top5.values()).toContain(0.8883)
    expect(value.prediction.top5.values()).toContain(0.0824)
    expect(value.prediction.top5.values()).toContain(0.0035)
    expect(value.prediction.top5.values()).toContain(0.0026)
    expect(value.prediction.top5.values()).toContain(0.0021)
    expect(value.detections.length).toEqual(6)
    expect(value.detections.map((det)=>det.category)).includes(Category.HUMAN)
    expect(value.detections.map((det)=>det.category)).includes(Category.VEHICLE)
